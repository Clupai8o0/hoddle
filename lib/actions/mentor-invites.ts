...
export async function inviteMentor(
  ...
  const noteHtml = note
    ? `<p style="color:#5a6275;font-size:15px;line-height:1.6;margin:24px 0 0;">${escapeHtml(note)}</p>`
    : "";
        ...
        <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;margin-top:32px;">
          <tr>
            <td style="padding:0 4px;">
              ${noteHtml}
              <p style="font-size:15px;color:#1a2035;line-height:1.6;margin:${noteHtml ? "24px" : "0"} 0 32px;">
                Click the button below to create your mentor account. The link expires on <strong>${expiryDate}</strong>.
              </p>
              <a href="${signupUrl}" style="display:inline-block;background-color:#001842;color:#f5f7fa;font-family:'Plus Jakarta Sans',system-ui,sans-serif;font-size:16px;font-weight:600;padding:16px 32px;border-radius:8px;text-decoration:none;">
                Accept invitation →
              </a>
